...
<input type ="hidden" name = "arrangeVocalsLength" value = "${arrangeVocalsCommandLength}"/>
...
<div class="contents">
	...
	<div class="arrRegList">
		...
		<div class="oh">
			<div class="coverArt">
				<a href="#none"><img src="../resources/images/holderType3.gif" alt="" style="width: 110px; height: 110px;"></a>
			</div>
			<div class="soundArea">
				<div class="soundHeader">
					<a href="#none" class="bulPlay" id = 'compose-play' data-sound-url="http://www.jplayer.org/audio/m4a/Miaow-07-Bubble.m4a;http://www.jplayer.org/audio/ogg/Miaow-07-Bubble.ogg">
				...
				<div class="soundBody">
					<div class="pajaImg">
						<img src="${command.arrangeCommand.arrangeSoundPng}" class="mb20" alt="" style="width: 100%;">
					</div>
		...
		<table class="tblType01 mt20 mb20">
			...
			<tr>
				<th>파일</th>
				<td>
					<a href="#none" data-score-url = "${command.compositionCommand.composeScoreWeb}" class="showScore" data-score-url=""> <span class="label label-score">작곡 악보</span></a>
					<a href="#none" data-sound-url = "${command.compositionCommand.composeSoundMp3};${command.compositionCommand.composeSoundOgg}">   <span class="label label-source">작곡 음원</span></a></td>
			</tr>
			<tr>
				<th>공개</th>
		...
		<div class="arrInfoWrap">
			<h4>작곡자 메모</h4>
			<textarea cols="50" readonly rows="5" placeholder="작곡자 메모 내용 삽입"style="width: 99%">
	...
	<div class="arrInfoWrap">
		<h4>편곡 제목</h4>
		<div class="mb20">
			<input type="text" value="${command.arrangeCommand.arrangeTitle}" readonly style="width: 99%">
		</div>
		<h4>
			장르<strong class="pointColor">(필수)</strong>
		</h4>
		<div class="mb20">
			<select class="dropdown" class="dropdown" name="arrangement.genreNo">
		...
		<h4>편곡 업로드<strong class="pointColor">(필수 1개 이상)</strong></h4>
		<div class="oh mb20">
			<div class="floatL">
		...
		<div class="soundHeader borderDiv mb30">
			...
			<div class="floatL ml10" style="width: 96%;"><img id = "arrange-frequency-holder" height = "70px" src="${command.arrangeCommand.arrangeSoundPng}" class="pl10" alt="" style="width: 95%;"></div>
		</div>
		...
		<h4>편곡자 메모</h4>
		<div class="mb20">
			<textarea cols="50" rows="5" placeholder="메모를 입력하세요" name = "arrangement.arrangeMemo" style="width: 99%">${command.arrangement.arrangeMemo}</textarea>
		</div>
		<h4>편곡 이미지</h4>
		<div class="floatL">
			<img id ="arrange-image-holder" src="${command.arrangeCommand.realArrangeImage}" alt="" style="width: 120px; height: 120px;">
		</div>
		<div class="imgUpload">
			<div class="floatL">
	...
	<div class="arrInfoWrap">
		...
		<div class="contBox01 mb20">
			<p class="bulList mb10" >배포 수를 지정하시면 당 편곡은 지정한 배포 수 만큼만 보컬에게 판매됩니다.</p>
				<input type="radio" name = "arrangement.limitStatus" class = "limitStatus "  <c:if test ="${command.arrangement.limitStatus  == 'N'}"> checked = "checked"</c:if> name ="arrangement.limitStatus" value = "N" id="unlimit"/><label for="unlimit">무제한</label>
				<input type="radio" name = "arrangement.limitStatus" class = "limitStatus"  <c:if test ="${command.arrangement.limitStatus  == 'Y'}"> checked = "checked"</c:if> name ="arrangement.limitStatus" value = "Y" id="limit"/><label for="limit">제한</label>
				<input type="text"  name = "arrangement.limitCount"  class = "limitCountBox" 
					<c:choose>
						<c:when test ="${command.arrangement.limitStatus  == 'N'}">
							disabled
						</c:when>
						<c:when test ="${command.arrangement.limitStatus  == 'Y'}">
						</c:when>
					</c:choose>
				value ="${command.arrangement.realLimitCount}" style="width: 100px; text-align: right;"> 명
		</div>
		<h4>배분율</h4>
		<div class="contBox01">
			<ul>
				...
				<li>
					...
					<input type="text"   id ="arrangeRate" name = "arrangement.arrangeRate" value = "${command.arrangement.arrangeRate}" style="width: 100px; text-align: right;"> &nbsp;%
					</p>
				</li>
			</ul>
		</div>
		...
		<div class="tabCont">
			<div class="arrInfoWrap mb20">
				...
				<table class="tblType02 addTrType2 mb20">
					...
					<c:forEach items = "{command.performanceCommands}" var = "performanceCommand" begin = "0" end = "${performanceCommandLength -1}" varStatus = "status">
					<tr>
						<td>
						...
						<td>
							<input type = "hidden" name = "performanceCommand[${status.index}].updateType" value = "${performanceCommand.updateType}" class ="updateType" />
							<input type = "hidden" name = "performanceCommand[${status.index}].messageToPerformance" value = "${performanceCommand.sendMessage}"/>
							<input type = "text" readonly class = 'selectedMemberBox' value = "${performanceCommand.performMemberName}" placeholder="선택한 연주자 이름" style="width: 200px;">
						</td>
						<td>
							<input type="text" name = "performances[${status.index}].performRate" value = "${performanceCommand.performRate}" style="width: 50px; text-align: right;" class ="performRate"> % 배분
						</td>
						<td>
				...
				<input type="text" name = "arrangement.lyricsRate" id ="lyricsRate" value ="${command.arrangement.lyricsRate}" style="width: 96px; text-align: right;" class ="compseRate" id ="composeRate">
				&nbsp;%&nbsp; 배분
			</div>
		</div>
		<!-- //tabBox1 -->
	...
	<div class="arrInfoWrap">
		<h4>보컬 공개 여부</h4>
		<div class="mb20">
			<input type="radio" name ="arrangement.openStatus" id="openVocal" 
				<c:if test ="${command.arrangement.openStatus  == 'Y' }">    checked = "checked"</c:if> value ="Y">
				<label for="openVocal">공개</label>
			<input type="radio" name ="arrangement.openStatus" id="closedVocal" 
				<c:if test ="${command.arrangement.openStatus  == 'N' }"> checked = "checked"</c:if> class="checkType1" value ="N">
			<label for="closedVocal">비공개</label>  
			<strong class="pointColor">(편곡의 보컬에 대한 공개 여부를 지정합니다.)</strong>
		</div>
		<c:set var = "arrangeVocalLength" value = "${fn:length(command.arrangeVocals)}"/>
		<input type ="hidden" id = "arrangeVocalLength" name = "arrangeVocalLength" value = "${arrangeVocalLength}"/>
		
		
		<div class ="vocalAddContainer">
			<div class="hiddenBoxType1" <c:if test ="${command.arrangement.openStatus  == 'Y'}"> style ="display:none"</c:if> >
			...
			<div class="floatL" style="width: 30%;">
				<table class="tblType02 addTrType1">
					...
					<c:if test = "${command.arrangement.openStatus == 'N' && arrangeVocalLength > 0}">
					<c:forEach items = "${command.arrangeVocals}" var = "arrangeVocal" begin = "0" 
						end = "${arrangeVocalLength-1}" varStatus ="status">
						<tr>
							<td>
							...
							<td>
								<input id = "selectedVocalBox" class = 'selectedMemberBox' type="text" 		value = "command.arrangeVocalNames[${status.index}]">
							</td>
							<td>
								<input type ="hidden" name = "arrangeVocals[${satus.index}].arrangeNo" 	    value ="command.arrangeVocals[${status.index}].arrangeNo">
								<input type ="hidden" name = "arrangeVocals[${satus.index}].vocalMemberNo"  value ="command.arrangeVocals[${status.index}].vocalMemberNo">
	...
	<div class="btnAreaType01">
		<div class="textR">
			<a class="btn large" href="#none"><span>취소</span></a> <a
				class="btn large btnPoint" href="#none"><span>등록</span></a>
		</div>
	</div>
						...
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '1' data-member-name = '매투릭스' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '2' data-member-name = '안영철' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '3' data-member-name = '김기수' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
					</table>
				...
				<div class="popCont">
					...
					<table class="tblType03 addTrType2">
						...
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '5' data-member-name = '홍길동' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '6' data-member-name = '임꺽쩡' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
						<tr>
							...
							<td>
								<a class="btn small btnPoint ctgView" href="#none">
									<span data-member-no = '7' data-member-name = '손수레' class = 'select-member'>select</span>
								</a>
							</td>
						</tr>
					</table>
...
<script type = "text/javascript">
	...
	$(document).ready(function(target) {
		...
		function checkRate(){
	    	...
	    	$('.tabWrapType1 tblType02 tr .performRate').each(function(){
	    ...
	    $(document).on('change', '#arrangeRate, #lyricsRate, #composeRate , .tabWrapType1 tblType02 tr .performRate', function(){
	    	checkRate(this);
	    });
	});
